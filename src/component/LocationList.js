...
const Home = () => {
    ...
    const fetchLocation = async () => {
        try {
            const res = await axios.post("http://192.168.2.194:5000/getlocationlist");
    ...
    const handleFetch = async () => {
        if (location.length > 1) {
            try {
                setFetchlocation([])
                setLoading(true);
                const data = await axios.post(`http://192.168.2.194:5000/fetchlocation`, { location });
    ...
    const handleAdd = async () => {
        try {
            const res = await axios.post("http://192.168.2.194:5000/addlocation", fetchlocation);
            if (res?.status === 500) {
    ...
    const deleteLocation = async (id) => {
        try {
            const res = await axios.post("http://192.168.2.194:5000/deletelocation", { _id: id });
            if (res?.status === 200) {
                alert(res?.data?.message)
                setFetchlocation([]);
                fetchLocation();
